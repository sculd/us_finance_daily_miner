...
recent_date_str = str(recent_date)

symbols_to_graph = []

def get_top_gainer_html(day):
    ...
    html_str += ((df_momentums[df_momentums.close > 20].xs(recent_date_str, level=0).sort_values(column, ascending=False).head()[['close', column]].rename(columns={column: 'return over {d} days'.format(d=display_day)}) * 1).round(3).astype(str) + '').to_html()
    html_str += '\n</div>\n'

    for symbol in list(df_momentums[df_momentums.close > 20].xs(recent_date_str, level=0).sort_values(column, ascending=False).head().index):
...
def get_top_loser_html(day):
    ...
    column = 'rtr{d}m'.format(d=day)
    html_str += ((df_momentums[df_momentums.close > 20].xs(recent_date_str, level=0).sort_values(column, ascending=True).head()[['close', column]].rename(columns={column: 'return over {d} days'.format(d=display_day)}) * 1).round(3).astype(str) + '').to_html()
    html_str += '\n</div>\n'

    for symbol in list(df_momentums[df_momentums.close > 20].xs(recent_date_str, level=0).sort_values(column, ascending=True).head().index):
        if symbol not in symbols_to_graph:
            symbols_to_graph.append(symbol)
...
html_str = ''
for d in days:
    html_str += get_top_gainer_html(d)

html_str += '<br clear="all" />'

for d in days:
    html_str += get_top_loser_html(d)

html_str += '<br clear="all" />'

html_str += add_graph_html(symbols_to_graph)
